...
- 全息启动与关闭：收到反馈时自下而上显现，关闭时从上往下收回，最后底部光环缩小熄灭，默认闲置 60 秒后隐藏；等待时间、速度、光感、扫描纹理和颜色均可调整，也可关闭后常驻。
- 呼吸、目光微调与六种回应动作，可设置幅度、频率和呼吸强度。
- 18 种在线音色，另加本机音色；支持语速、音高和音量调节。
- 朗读当前 Codex 任务的公开进度与最终回答；v0.4.2 自动跟随同一任务的日志轮换，断开后自动重连，避免重播旧消息。
- 摄像头人脸跟随：偶尔注视或持续注视。
...
打包输出 `dist/Companion-v0.4.5/Rin.exe`，可通过 `启动凛.vbs` 启动。当前打包不内置 Python 解释器；发布给其他电脑时仍需安装 Python 和语音依赖。
