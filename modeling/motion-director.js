...
export class MotionDirector{
 ...
 trigger(kind,time,intensity){this.gesture={kind,intensity,start:time,duration:kind==='wave'?3.6:kind==='stretch'?4.5:2.8};}
 ...
 if(!speaking&&time>(this.plannedUntil||0)&&s.frequency>0&&time>this.next){this.next=time+8+(1-s.frequency)*22+Math.random()*10;this.trigger(speaking?['nod','explain','tilt'][Math.floor(Math.random()*3)]:['look','tilt','settle'][Math.floor(Math.random()*3)],time)}
 const g=this.gesture;if(!g)return;const p=(time-g.start)/g.duration;if(p>=1){this.gesture=null;return}const e=Math.sin(Math.PI*Math.max(0,p))**2*(g.intensity??amount);
 ...
 if(g.kind==='wave')this.waveArm(Math.sin(Math.PI*Math.max(0,p))**2*Math.min(1,(g.intensity??amount)*1.7),p);
