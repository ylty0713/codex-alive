...
v0.4.0：白底黑白控制中心、角色/场景/感知/声音四分区。新增可配置的全息出现与消散：默认闲置 60 秒，反馈到来时唤醒，语音播放期间保持显示。出现时按世界高度裁切并加入发光扫描边缘，消散时按屏幕空间噪声逐渐移除像素；保留原角色材质。可调颜色、扫描强度、时长与背景。呼吸、目光微动作、点头、挥手、倾听、环顾、讲述与舒展叠加到原动作，挥手使用手臂二骨 IK；属于程序化动作层，尚非完整动作捕捉。在线音色扩展为 18 种，语速/音高/音量可调，本机音色另计。

壁纸清除 Win32 非客户区边框与扩展边缘，禁用圆角，并在窗口显示后进行最终定位。验证同时检查外框和客户区的屏幕坐标及尺寸；本机 1440×2560 与 1920×1080 两块显示器均完全匹配。验证见 scripts/verify-v04.cjs、tests/experience.test.mjs；另回归了真实进度朗读和隐藏控制面板时的口型同步。
